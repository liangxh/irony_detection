...
class NNModel(BaseNNModel):
    ...
    def build_neural_network(self, lookup_table):
        ...
        with tf.variable_scope("blstm_layer_0"):
        ...
        with tf.variable_scope("blstm_layer_1"):
            cell_fw = rnn_cell.build_lstm(self.config.rnn_dim, dropout_keep_prob=dropout_keep_prob)
            cell_bw = rnn_cell.build_lstm(self.config.rnn_dim, dropout_keep_prob=dropout_keep_prob)
        ...
        with tf.variable_scope("blstm_layer_2"):
            cell_fw = rnn_cell.build_lstm(self.config.rnn_dim, dropout_keep_prob=dropout_keep_prob)
            cell_bw = rnn_cell.build_lstm(self.config.rnn_dim, dropout_keep_prob=dropout_keep_prob)
...
def train(text_version='ek', label_version=None, config_path='config93_naive.yaml'):
    """
    python -m algo.main93 train semeval2019_task3_dev
    python3 -m algo.main93 train semeval2019_task3_dev -c config_ntua93.yaml
