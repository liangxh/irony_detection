...
def tf_idf(dataset_key, text_version, label_version=None, use_class_weights=True):
    """
    python algo/svm.py tf_idf semeval2019_task3_dev -t ek
    """
    ...
    vectorizers = {
        ...
        #TF: CountVectorizer(tokenizer=lambda x: x.split(' '), ngram_range=(1, 3), min_df=0.02, max_features=1000),
        #TF_C: TfidfVectorizer(ngram_range=(1, 1), analyzer='char', lowercase=False, smooth_idf=True, sublinear_tf=True)
        TF_C: TfidfVectorizer(ngram_range=(1, 3), analyzer='char', lowercase=False, smooth_idf=True, sublinear_tf=True)
